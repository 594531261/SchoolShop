...
//				+ formatFromSize(free));
	...
	public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
		...
		}else if(cacheDir.isDirectory() && cacheDir.canWrite() ){
			return new DiskLruCache(cacheDir, maxByteSize/2);
		}else{
			return new DiskLruCache(cacheDir, 1*1024*1024);
		}
	}
	...
	private void flushCache() {
		...
		long eldestFileSize;
		int count = 0;
		int maxCacheByteSize = 100 * 1024 * 1024;
	...
	public Bitmap get(String key) {
		synchronized (mLinkedHashMap) {
			final String file = mLinkedHashMap.get(key)+".tmp";
			if (file != null) {
				if (BuildConfig.DEBUG) {
				...
				final String existingFile = createFilePath(mCacheDir, key) +".tmp";;
...
//				.getExternalCacheDir(context).getPath() : context.getCacheDir().getPath();
		final String cachePath = getPath();
