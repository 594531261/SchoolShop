...
import com.school.schoolclient.BuildConfig;
...
public class ImageCache {
    ...
    private static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 1024 * 20; // 5MB
    ...
    private void init(Context context, ImageCacheParams cacheParams) {
        ...
        if (cacheParams.memoryCacheEnabled) {
            mMemoryCache = new LruCache<String, WeakReference<Bitmap>>(cacheParams.memCacheSize) {
                /**
...
//                	final Bitmap bt = bitmap.get();
//                	if(bt != null && !bt.isRecycled()){
//                		return Utils.getBitmapSize(bt);
//                	}else{
//                		return 1;
//                	}
                }
    ...
    public void addBitmapToCache(String data, Bitmap bitmap) {
        ...
        // Add to memory cache
        if (mMemoryCache != null && (mMemoryCache.get(data) == null || mMemoryCache.get(data).get() == null)) {
        	WeakReference<Bitmap> wb = new WeakReference<Bitmap>(bitmap);
            mMemoryCache.put(data, wb);
        }
    ...
    public Bitmap getBitmapFromMemCache(String data) {
        if (mMemoryCache != null) {
        	WeakReference<Bitmap> wb =  mMemoryCache.get(data);
        	if(wb != null){
        		final Bitmap memBitmap = wb.get();
        		if (memBitmap != null) {
